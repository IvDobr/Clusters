...
public class Clusters {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Point> points = new ArrayList<>(); //создаем массив всех точек
        ...
        ArrayList<Cluster> clasters = new ArrayList<>(); //создаем массив кластеров
        ...
        for (int i = 1; i < n; i++) { //Главный цикл - перебираем точки со второй
            System.out.println("==================================================");
            System.out.println("Итерация " + i);

            TreeMap<Integer, Double> treeMap = new TreeMap<>(); //массив номеров кластеров, расстояние к которым у точки меньше PSI

            for (int j = 0; j < clasters.size(); j++) { //перебираем все кластеры, а внутри берем у каждого центр тяжести
                System.out.println("В точке " + points.get(i).toString() + " расстояние до кластера " + j + ": " + df.format(distance(clasters.get(j).cg(), points.get(i))));
            ...
            if (!treeMap.isEmpty()) { //Проверяем, пустой ли массив с подходящими кластерами
                ...
                System.out.println("Минимальное расстояние: "+ df.format(treeMap.get(treeMap.firstKey())) + ". Кладем точку в кластер " + treeMap.firstKey());
            } else {
            ...
            System.out.println("------------------------------------------");
            System.out.println("Выведем центры тяжести кластеров и точки в них");
            for (int j = 0; j < clasters.size(); j++) {
                System.out.println("-------");
                System.out.println("Ц. Т. кластера " + j + " - " + clasters.get(j).cgToString());
                System.out.println("Он содержит точки:");
                for (Point pt : clasters.get(j).getPoints()) {
             ...
             */
            System.out.println("------------------------------------------");
            System.out.println("Начинаем процесс объединения кластеров" );
